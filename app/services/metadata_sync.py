    ...
    while True:
        ...
        for job in response["results"]:
            ...
            await db.execute(
                ...
                (job_dict.get("location", {}).get("name") if job_dict.get("location") else None),
            ...
            await db.execute("DELETE FROM job_interview_plans WHERE job_id = $1", job_dict["id"])
...
async def sync_interview_stages() -> None:
    ...
    plans = await db.fetch("SELECT interview_plan_id FROM interview_plans WHERE NOT is_archived")
